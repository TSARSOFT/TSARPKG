...
def archinstall():
    pkg=custominstaller.get()
    os.system('pacman -S '+pkg+' --noconfirm')

def debinstall():
    pkg=custominstaller_deb.get()
...
select=tkinter.OptionMenu(install, default, 'TSARED', 'TSARPKG', 'TSARCTL')
select.grid(row=0, column=0)

tkinter.Button(install, text='install/update', command=lambda:installprog(default.get())).grid(row=0, column=1)

custominstaller=tkinter.Entry(install)
custominstaller.grid(row=1, column=1, columnspan=2)
custominstaller_deb=tkinter.Entry(install)
custominstaller_deb.grid(row=2, column=1, columnspan=2)
tkinter.Button(install, text='arch linux install', command=lambda:archinstall()).grid(row=1, column=0)
tkinter.Button(install, text='debian/ubuntu linux install', command=lambda:debinstall()).grid(row=2, column=0)
